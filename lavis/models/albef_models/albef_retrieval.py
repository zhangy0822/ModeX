...
from lavis.models.albef_models import AlbefBase, compute_sim_matrix, Modex
...
class AlbefRetrieval(AlbefBase, MomentumDistilationMixin, SharedQueueMixin):
    ...
    def from_config(cls, cfg=None):
        ...
        image_encoder = VisionTransformerEncoder.from_config(cfg, from_pretrained=False, peft_config=peft_config_image, modex=modex)
        ...
        model.set_required_grad_setting(peft_config_image, peft_config_text, task='retrieval', modex=modex)
        ...
        model_info['n_vision_proj'] = sum(p.numel() for p in model.vision_proj.parameters())
        model_info['n_text_proj'] = sum(p.numel() for p in model.text_proj.parameters())
        model_info['n_itm_head'] = sum(p.numel() for p in model.itm_head.parameters())
        model_info['n_temp'] = model.temp.numel()
        model_info['n_params_query_model'] = model_info['n_visual_encoder'] + model_info['n_textual_encoder'] + model_info['n_vision_proj'] + model_info['n_text_proj']
        model_info['trainable_params_ratio'] = model_info['n_trainable_params'] * 100 / (model_info['n_params_query_model'] + model_info['n_itm_head'] + model_info['n_temp'])
